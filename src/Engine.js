...
class Engine {
  ...
  generation() {
    ...
    const start = (transformedCluster, TREE_DEPTH_LIMIT, RESULT_LIMIT, STACK_LIMIT, DEATH_TIME) => {
      while (
        totalDepth.getCurrent() < TREE_DEPTH_LIMIT &&
        res.size < RESULT_LIMIT &&
        stack[0].length + stack[1].length < STACK_LIMIT
      ) {
        if (Date.now() - timeStart >= DEATH_TIME) break;
        let curStr = stack[0].shift();

        let { chains, words } = this.system.replaceAllDeterminate(
          curStr,
          transformedCluster,
          this.options.determinate,
          this.system.includes,
          STACK_LIMIT - stack[0].length - stack[1].length,
          RESULT_LIMIT - res.length
        );

        if (chains.length === 0 && words.length === 0) {
          //Если никаких изменений не произошло, значит это готове слово;
          res.add(curStr);
        }
    ...
    start(
      this.options.rules.transformedCluster,
      this.settings.TREE_DEPTH_LIMIT,
      this.settings.RESULT_LIMIT,
      this.settings.STACK_LIMIT,
      this.settings.DEATH_TIME
    );
