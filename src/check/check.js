export function correct_grammar_check(obj_rule) {
  ...
  for (let key in obj_rule) {
    ...
    if (typeof obj_rule[key] === 'string') {
      //Если тип строка, то пробегаем по всем ее символам и проверяем - есть ли хоть один символ, который не равен ключу
      flag = true;
  ...
  for (let key in obj_rule) {
    //Собираем все элементы, где есть переход в нетерминал
    flag = false;
    ...
    if (typeof obj_rule[key] === 'string') {
      ...
      if (flag) {
        non_terminal_check_arr.push(key);
      }
      ...
      for (let i = 0; i < obj_rule[key].length; i++) {
        for (let j = 0; j < obj_rule[key][i].length; j++) {
          if (non_terminal_all_arr.includes(obj_rule[key][i][j])) {
            flag = true;
          }
        }
      }
  ...
  for (let w = 0; w < non_terminal_check_arr.length; w++) {
    ...
    while (flag) {
      ...
      for (let i = 0; i < non_terminal_check_arr.length; i++) {
        if (typeof obj_rule[non_terminal_check_arr[i]] === 'string') {
          for (let j = 0; j < obj_rule[non_terminal_check_arr[i]].length; j++) {
            if (
              obj_rule[non_terminal_check_arr[i]][j] !== non_terminal_check_arr[w] &&
          ...
          for (let j = 0; j < obj_rule[non_terminal_check_arr[i]].length; j++) {
            for (let z = 0; z < obj_rule[non_terminal_check_arr[i]][j].length; z++) {
              if (
                obj_rule[non_terminal_check_arr[i]][j][z] !== non_terminal_check_arr[w] &&
                !check_arr.includes(obj_rule[non_terminal_check_arr[i]][j][z])
              ) {
                flag_2 = false;
              }
            }
      ...
      if (save === check_arr.length || check_arr.length > 20) {
        flag = false;
      }
    }
    if (typeof obj_rule[non_terminal_check_arr[w]] === 'string') {
      ...
      for (let i = 0; i < obj_rule[non_terminal_check_arr[w]].length; i++) {
        for (let j = 0; j < obj_rule[non_terminal_check_arr[w]][i].length; j++) {
          if (check_arr.includes(obj_rule[non_terminal_check_arr[w]][i][j])) {
            flag_2 = false;
          }
        }
...
export function CHF(obj_rule) {
  //Фунция получает набор правил в виде одного объекта и мутирует его
      ...
      if (typeof obj_rule[key2] === 'object') {
        for (let i = 0; i < obj_rule[key2].length; i++) {
          if (obj_rule[key2][i] === key) {
            if (obj_rule[key] === key2) {
              obj_rule[key2].splice(i, i);
...
export function one_noterm(obj_rule) {
  ...
  while (flag) {
    ...
    for (let key in obj_rule) {
      if (
        obj_rule[key].length === 1 &&
        typeof obj_rule[key] === 'string' &&
        key !== 'S0'
      ) {
        for (let i = 0; i < no_term_arr.length; i++) {
          if (
            obj_rule[key] === no_term_arr[i] &&
            key !== no_term_arr[i] &&
            key !== 'S0'
          ) {
            flag = true;
        ...
        for (let j = 0; j < obj_rule[key].length; j++) {
          if (obj_rule[key][j].length === 1) {
            for (let i = 0; i < no_term_arr.length; i++) {
              if (
                obj_rule[key][j] === no_term_arr[i] &&
                key !== no_term_arr[i] &&
                key !== 'S0'
              ) {
                flag = true;
                obj_rule[key][j] = go_to_norm_rule(
                  obj_rule,
                  no_term_arr,
                  obj_rule[key][j]
                );
              ...
              } else if (obj_rule[key][j] === no_term_arr[i] && key === no_term_arr[i])
                return 0;
            }
          }
        }
...
export function go_to_norm_rule(obj_rule, no_term_arr, elem) {
  ...
  if (obj_rule[elem].length > 1 || obj_rule[elem] === undefined) return obj_rule[elem];
    ...
    if (typeof obj_rule[elem] === 'string') {
      for (let i = 0; i < no_term_arr.length; i++) {
        if (obj_rule[elem] === no_term_arr[i] && elem !== no_term_arr[i])
          save = go_to_norm_rule(obj_rule, no_term_arr, obj_rule[elem]);
        else return;
      }
      if (save === undefined) return obj_rule[elem];
    } else {
      for (let i = 0; i < no_term_arr.length; i++) {
        if (obj_rule[elem][0] === no_term_arr[i])
          save = go_to_norm_rule(obj_rule, no_term_arr, obj_rule[elem][0]);
      }
      if (save === undefined) return obj_rule[elem];
    }
  }
  ...
  while (flag) {
    ...
    for (let key in obj_rule) {
      if (obj_rule[key].length > 2 && typeof obj_rule[key] === 'string') {
        ...
        for (let key2 in obj_rule) {
          if (obj_rule[key2] === save_string && !alf_NT_old.includes(key2))
            repeat_def = key2;
        }
        if (repeat_def === undefined && flag) {
          while (true) {
            if (alf.includes(`${counter}`, 0) === false) {
              if (counter < 10)
                string = obj_rule[key].slice(0, counter_tild + 1) + `${counter}`;
        ...
        for (let i = 0; i < obj_rule[key].length; i++) {
          if (obj_rule[key][i].length > 2) {
            ...
            if (obj_rule[key][i][0] !== '~')
              ...
              save_string = obj_rule[key][i].slice(
                counter_tild + 2,
                obj_rule[key][i].length
              );
              flag = false;
              counter_tild += 1;
            ...
            if (repeat_def === undefined) {
              while (true) {
                if (alf.includes(`${counter}`, 0) === false) {
                  if (counter < 10)
                    string = obj_rule[key][i].slice(0, counter_tild + 1) + `${counter}`;
                  else
                    string = obj_rule[key][i].slice(0, counter_tild + 1) + `~${counter}~`;
...
export function some_term_sign(obj_rule) {
  ...
  for (let key in obj_rule) {
    if (typeof obj_rule[key] === 'string') {
      for (let i = 0; i < obj_rule[key].length; i++) {
        if (
          obj_rule[key][i] in obj_rule === false &&
          alf.includes(obj_rule[key][i], 0) === false
        )
          alf.push(obj_rule[key][i]);
        else if (
          obj_rule[key][i] in obj_rule === true &&
          alf_NT_old.includes(obj_rule[key][i], 0) === false
        )
          alf_NT_old.push(obj_rule[key][i]);
      }
    } else if (typeof obj_rule[key] === 'object') {
      for (let i = 0; i < obj_rule[key].length; i++) {
        for (let j = 0; j < obj_rule[key][i].length; j++) {
          if (
            obj_rule[key][i][j] in obj_rule === false &&
            alf.includes(obj_rule[key][i][j], 0) === false
          )
            alf.push(obj_rule[key][i][j]);
          else if (
            obj_rule[key][i][j] in obj_rule === true &&
            alf_NT_old.includes(obj_rule[key][i][j], 0) === false
          )
            alf_NT_old.push(obj_rule[key][i][j]);
        }
      }
    }
  }
    ...
    if (typeof obj_rule[key] === 'string' && obj_rule[key].length > 1) {
      for (let i = 0; i < obj_rule[key].length; i++) {
        ...
        if (check_term === true) {
          ...
          for (let key2 in obj_rule) {
            if (obj_rule[key2] === save_term && !alf_NT_old.includes(key2))
              repeat_def = key2;
          }
          if (repeat_def === undefined) {
            while (true) {
              if (alf.includes(`${index}`, 0) === false) {
                if (index < 10) {
                  string =
                    obj_rule[key].slice(0, i) +
                    `${index}` +
                    obj_rule[key].slice(i + 1, obj_rule[key].length);
                } else {
                  string =
                    obj_rule[key].slice(0, i) +
                    `~${index}~` +
              ...
              if (repeat_def === undefined) {
                while (true) {
                  if (alf.includes(`${index}`, 0) === false) {
                    if (index < 10) {
                      string =
                        obj_rule[key][i].slice(0, j) +
                        `${index}` +
                        obj_rule[key][i].slice(j + 1, obj_rule[key][i].length);
                    } else {
                      string =
                        obj_rule[key][i].slice(0, j) +
                        `~${index}~` +
...
export function del_epsilon(obj_rule) {
  //Функция удаляет пустоту из набора правил
  let del_el = [];
  let flag = true;
  let flag_2 = true;
  let flag_3 = true;
  ...
  for (let key in obj_rule) {
    ...
    //Если правило является пустой строкой, то добавляем это правило в массив элементов для удаления
    else if (typeof obj_rule[key] === 'object') {
      //Если у нетернара несколько правил
      flag = false;
      flag_2 = true;
      for (let i = 0; i < obj_rule[key].length; i++) {
        //Пробегаемся по всем правилам
  ...
  flag = true;
  while (flag) {
    //Будем выполнять цикл, пока на каждой итерации находится хотя бы один элемент, который является пустым
    flag_2 = false;
    for (let key in obj_rule) {
      //Пробегаемся по всем правилам
      if (typeof obj_rule[key] === 'string' && obj_rule[key].length === 1) {
        //Если нетернар имеет только одно правило перехода и количество символов после перехода равна 1
        for (let j = 0; j < del_el.length; j++) {
          //Пробегаемся по массиву элементов для удаления
          if (obj_rule[key] === del_el[j] && key !== 'S0') {
            //Если нетернар переходит в один из таких элементов и этот нетернар не является первым
            for (let k = 0; k < del_el.length; k++) {
              //снова пробегаемся по массиву с эл-ми для удаления
              if (key === del_el[k]) string = false; //Если такой элемент уже был в массиве, то пропускаем его
            }
            if (string === undefined) {
              //Если такого элемента не было, то добавляем его в массив и даем понять, что while должен быть выполнен еще раз
              flag_2 = true;
              del_el.push(key);
            }
            string = undefined;
          } else if (obj_rule[key] === del_el[j] && key === 'S0')
            obj_rule[key] = ['S', ''];
        }
      } else if (typeof obj_rule[key] === 'string' && obj_rule[key].length > 1) {
        //Если нетернар имеет только одно правило перехода и количество символов после перехода более 1
        flag_3 = false;
        flag_4 = true;
        for (let i = 0; i < obj_rule[key].length; i++) {
          //Пробегаемся по всей строке перехода данного нетернара
          for (let j = 0; j < del_el.length; j++) {
            //Пробегаемся по массиву с эл-ми для удаления
        ...
        if (flag_4) {
          //Если все элементы строки, в которую переходит нетернар, в свою очередь переходят только в пустоту
          for (let k = 0; k < del_el.length; k++) {
            //Проверяем был ли такой нетернар уже в массиве
        ...
        //Если нетернар переходит в несколько правил
        for (let i = 0; i < obj_rule[key].length; i++) {
          //Пробегаемся по всем правилам
          flag_3 = false;
          flag_4 = true;
          for (let j = 0; j < del_el.length; j++) {
            if (obj_rule[key][i].length === 1) {
              //Если одно из правил - переход в один символ
              if (obj_rule[key][i] === del_el[j] && key !== 'S0') flag_3 = true;

              if (
                obj_rule[key][i] !== key &&
                obj_rule[key][i] !== '' &&
                obj_rule[key][i] !== del_el[j]
              ) {
                flag_4 = false;
                for (let k = 0; k < del_el.length; k++) {
              ...
              //Если правило - переход в несколько символов ////////////////ОБНОВА//////////////////
              for (let z = 0; z < obj_rule[key][i].length; z++) {
                //Пробегаемся по всем этим символам
  ...
  if (del_el.includes('S')) {
    obj_rule['S0'] = ['S', ''];
  }

  for (let i = 0; i < del_el.length; i++) {
    //Удаляем все нетернары, которые оказались в массиве на удаление
      ...
      for (let i = 0; i < obj_rule[key].length; i++) {
        for (let j = 0; j < del_el.length; j++) {
          if (obj_rule[key][i] === del_el[j]) {
            //Если в строке есть нетернар из массива на удаление
            string =
              obj_rule[key].slice(0, i) +
              obj_rule[key].slice(i + 1, obj_rule[key].length);
            obj_rule[key] = string;
        ...
        for (let j = 0; j < obj_rule[key][i].length; j++) {
          for (let k = 0; k < del_el.length; k++) {
            if (obj_rule[key][i][j] === del_el[k] && obj_rule[key][i].length > 1) {
              ...
              obj_rule[key][i][j] === del_el[k] &&
              obj_rule[key][i].length === 1
            ) {
  ...
  for (let key in obj_rule) {
    if (typeof obj_rule[key] === 'object' && key !== 'S0') {
      for (let i = 0; i < obj_rule[key].length; i++) {
        if (obj_rule[key][i] === '') {
          //Если нетернар переходит в пустоту
...
export function CYK_algorithm(arr_rule, word) {
  ...
  if (word.length === 0 && arr_rule['S0'].length === 2) {
      ...
      for (let j = 0; word.length > j; j++) {
        ...
        if (i === 0) {
          for (let k = 0; word.length > k; k++) {
            for (let key in arr_rule) {
              if (arr_rule[key].length < 2 && word[k] === arr_rule[key])
                Intermediate_arr.push(key);
          ...
          while (save_i >= 1) {
            if (
              CYK_arr_row[i - save_i] !== undefined &&
              CYK_arr_row[i - save_i][j] !== undefined &&
              CYK_arr_row[i - save_i][j].length !== 0 &&
              CYK_arr_row[i - (i - save_i + 1)] !== undefined &&
              CYK_arr_row[i - (i - save_i + 1)][j + (i - save_i + 1)] !== undefined
            ) {
              if (typeof CYK_arr_row[i - (i - save_i + 1)][j + save_j] === 'string') {
                if (typeof CYK_arr_row[i - save_i][j] === 'string') {
                  res_string =
                    CYK_arr_row[i - save_i][j] +
                    CYK_arr_row[i - (i - save_i + 1)][j + save_j];
                ...
                for (
                  let k = 0;
                  k < CYK_arr_row[i - (i - save_i + 1)][j + save_j].length;
                  k++
                ) {
                  if (typeof CYK_arr_row[i - save_i][j] === 'string') {
                    res_string =
                      CYK_arr_row[i - save_i][j] +
                      CYK_arr_row[i - (i - save_i + 1)][j + save_j][k];
                    ...
                    for (let z = 0; z < CYK_arr_row[i - save_i][j].length; z++) {
                      res_string =
                        CYK_arr_row[i - save_i][j][z] +
                        CYK_arr_row[i - (i - save_i + 1)][j + save_j][k];

                      Intermediate_arr = rule_check(
                        arr_rule,
                        res_string,
                        Intermediate_arr
                      );
    ...
    if (typeof CYK_arr_row[CYK_arr_row.length - 1][0] === 'string') {
      if (
        CYK_arr_row[CYK_arr_row.length - 1][0] === 'S0' ||
        CYK_arr_row[CYK_arr_row.length - 1][0] === 'S'
      ) {
        // console.log("Данное слово задано верно")
        return 1;
      ...
      for (let i = 0; CYK_arr_row[CYK_arr_row.length - 1][0].length > i; i++) {
        if (
          CYK_arr_row[CYK_arr_row.length - 1][0][i] === 'S0' ||
          CYK_arr_row[CYK_arr_row.length - 1][0][i] === 'S'
        ) {
          //console.log("Данное слово задано верно");
          return 1;
        }
        ...
        for (let key in obj_rule) {
          if (typeof obj_rule[key] === 'string' && obj_rule[key].length > 1) {
            ...
            if (obj_rule[key][0] === '~') {
              ...
              if (
                arr[[k, j, obj_rule[key].slice(0, counter + 1)]] === true &&
                arr[
                  [
                    i - k - 1,
                    j + k + 1,
                    obj_rule[key].slice(counter + 1, obj_rule[key].length)
                  ]
                ] === true
              ) {
                arr[[i, j, key]] = true;
              ...
              if (
                arr[[k, j, obj_rule[key][0]]] === true &&
                arr[
                  [i - k - 1, j + k + 1, obj_rule[key].slice[(1, obj_rule[key].length)]]
                ] === true
              ) {
                arr[[i, j, key]] = true;
              }
              ...
              if (
                arr[[k, j, obj_rule[key][0]]] === true &&
                arr[[i - k - 1, j + k + 1, obj_rule[key][1]]] === true
              ) {
                arr[[i, j, key]] = true;
              }
            }
            ...
            for (let z = 0; z < obj_rule[key].length; z++) {
              if (obj_rule[key][z].length > 1) {
                ...
                if (obj_rule[key][z][0] === '~') {
                  ...
                  if (
                    arr[[k, j, obj_rule[key][z].slice(0, counter + 1)]] === true &&
                    arr[
                      [
                        i - k - 1,
                        j + k + 1,
                        obj_rule[key][z].slice(counter + 1, obj_rule[key][z].length)
                      ]
                    ] === true
                  ) {
                    arr[[i, j, key]] = true;
                  ...
                  if (
                    arr[[k, j, obj_rule[key][z][0]]] === true &&
                    arr[
                      [
                        i - k - 1,
                        j + k + 1,
                        obj_rule[key][z].slice[(1, obj_rule[key][z].length)]
                      ]
                    ] === true
                  ) {
                    arr[[i, j, key]] = true;
                  ...
                  if (
                    arr[[k, j, obj_rule[key][z][0]]] === true &&
                    arr[[i - k - 1, j + k + 1, obj_rule[key][z][1]]] === true
                  ) {
                    arr[[i, j, key]] = true;
                  }
                }
...
export function Unambiguous_conversion(obj_rule, max_counter = 2) {
  ...
  while (flag) {
    ...
    substitution_unambiguous_rules(
      Unambiguous_rule,
      intermediate_rule,
      obj_rule,
      counter
...
function substitution_unambiguous_rules(
  Unambiguous_rule,
  intermediate_rule,
  obj_rule,
  counter
...
function Search_for_uniqueness(obj_rule, Unambiguous_rule) {
  ...
  for (let key in obj_rule) {
    ...
    if (typeof obj_rule[key] === 'string') {
      ...
      flag = true;
      for (let i = 0; i < obj_rule[key].length; i++) {
        //Проверяем каждый элемент строки на тернарность
        for (let key2 in obj_rule) {
          if (obj_rule[key][i] === key2) {
            flag = false;
          }
      ...
      if (flag) {
        //Если все элементы строки тернары
        for (let key2 in Unambiguous_rule) {
          for (let i = 0; i < Unambiguous_rule[key2].length; i++) {
        ...
        if (flag) {
          for (let key2 in Unambiguous_rule) {
            for (let j = 0; j < Unambiguous_rule[key2].length; j++) {
              if (obj_rule[key][i] === Unambiguous_rule[key2][j] && key === key2)
                flag = false;
            }
          }
          if (flag) {
